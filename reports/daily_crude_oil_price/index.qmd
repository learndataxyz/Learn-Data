...
oil_data_raw <- read_csv("Export.csv") |>
  mutate(
    Date = dmy(Date),
...
summary_table_gt <- summary_table %>%
  gt() %>%
  tab_header(
    title = md("**Summary of Nigeria Daily Crude Oil Data**"),
  ...
  tab_source_note(
    source_note = "Source: Central Bank of Nigeria"
  )

summary_table_gt
gtsave(summary_table_gt, "images/summary_table_gt.png")
```
...
median_price_hist <- ggplot(oil_data, aes(x = Price)) +
  geom_histogram(
    binwidth = 5,
  ...
  labs(
    ...
    y = "Number of Days",
    caption = "Source: Central Bank of Nigeria"
  ) +
...
ggsave("images/median_price_hist.png", median_price_hist)
```
...
# Create the plot
daily_line_chart <- ggplot(oil_data, aes(x = Date, y = Price)) +
  ...
  # Titles and labels
  labs(
    title = "Daily Nigeria Crude Oil Prices (2008–2025)",
...
daily_line_chart
ggsave("images/daily_line_chart.png", daily_line_chart)
```
...
```{r}
moving_average <- ggplot(oil_data, aes(x = Date)) +
  ...
  labs(
    title = "Daily Nigeria Crude Oil Prices\nwith 30-Day Moving Average",
...
moving_average
ggsave("images/moving_average.png", moving_average)
```
...
yearly_average <- ggplot(yearly_avg, aes(
  x = reorder(as.factor(Year), Avg_Price),
...
yearly_average
ggsave("images/yearly_average.png",yearly_average)
```
...
```{r}
rolling_volatility <- ggplot(oil_data, aes(x = Date, y = Volatility_30d)) +
  geom_line(color = "purple", size = 1) +
  labs(
    title = "30-Day Rolling Volatility of\nNigeria Crude Oil Prices",
...
rolling_volatility
ggsave("images/rolling_volatility.png",rolling_volatility)
...
monthly_avg <- oil_data %>%
  ...
  summarise(Avg_Price = mean(Price, na.rm = TRUE), .groups = "drop")
# Create the heatmap
monthly_average_heatmap <- ggplot(monthly_avg, aes(x = as.factor(Year), y = Month, fill = Avg_Price)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "lightyellow", high = "darkblue", name = "Avg Price") +
  labs(
    title = "Monthly Average Nigeria\nCrude Oil Prices Heatmap",
...
monthly_average_heatmap
ggsave("images/monthly_average_heatmap.png",monthly_average_heatmap)
...
oil_data_with_median <- oil_data %>%
  left_join(month_medians, by = "Month")

# Create the improved boxplot
median_plot <- ggplot(oil_data_with_median, aes(x = Month, y = Price, fill = Median_Price)) +
...
median_plot
ggsave("images/median_plot.png",median_plot)
...
perc_change_distr <- ggplot(oil_data, aes(x = Daily_Pct_Change)) +
  geom_histogram(
    binwidth = 1,
    fill = "#1f77b4", # professional blue
    color = "white",
...
perc_change_distr
ggsave("images/perc_change_distr.png",perc_change_distr)
...
top_swings_gt <- top_swings %>%
  ...
  data_color(
    ...
    colors = scales::col_numeric(
      ...
      domain = c(
        min(top_swings$Daily_Pct_Change),
...
top_swings_gt
gtsave(top_swings_gt, "images/top_swings_gt.png")
```
...
```{r}
volatility_curve <- ggplot(oil_data, aes(x = Date, y = Volatility_30d)) +
  geom_line(color = "purple", size = 1) +
  labs(
...
volatility_curve
ggsave("images/volatility_curve.png", volatility_curve)
...
yearly_summary_gt <- yearly_summary %>%
  ...
  data_color(
    ...
    colors = scales::col_numeric(
      palette = c("#d73027", "#ffffbf", "#1f77b4"), # Red → Yellow → Blue
      domain = c(
        min(yearly_summary$YoY_Change, na.rm = TRUE),
...
yearly_summary_gt
gtsave(yearly_summary_gt, "images/yearly_summary_gt.png")
...
monthly_summary_gt <- monthly_summary %>%
  ...
  data_color(
    ...
    colors = scales::col_numeric(
      palette = c("#d73027", "#ffffbf", "#1f77b4"), # Light yellow → Dark blue
  ...
  tab_source_note(
    source_note = paste0("Source: Central Bank of Nigeria | Year: ", latest_year)
  )

monthly_summary_gt
gtsave(monthly_summary_gt, "images/monthly_summary_gt.png")
```
